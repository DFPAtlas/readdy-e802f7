...
function auditEvent(
  ...
  try {
    supabaseAdmin.from("admin_security_audit_log").insert({
      actor_id: actorId,
      action,
      module: "email-ai-assistant",
      result: "denied",
      reason,
      success: false,
      details: { endpoint: "email-ai-assistant", ...extra },
...
serve(async (req: Request) => {
  ...
  if (!profile || !isActiveAdmin || isSuspended || isArchived || !isAllowedRole) {
    auditEvent(supabaseAdmin, callerId, "email_ai_access_rejected", "insufficient_privileges");
  ...
  try {
    ...
    if (orgOverride || organisation_id) {
      const { data: settings } = await supabaseAdmin.from("email_studio_settings").select("ai_config").maybeSingle();
    ...
    if (brandId) {
      const { data: brand } = await supabaseAdmin.from("email_brand_kits").select("name, product_name, description, ai_voice_settings").eq("id", brandId).maybeSingle();
      if (brand) {
    ...
    try {
      await supabaseAdmin.from("email_ai_usage_logs").insert({
