...
export default function FinalCTASection() {
  return (
    <section className="py-24 px-6 bg-[#060F1E] relative overflow-hidden">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(circle at 50% 50%, rgba(59,130,246,0.07) 0%, transparent 60%)' }} />
      <div className="absolute inset-0 opacity-[0.01]" style={{ backgroundImage: 'radial-gradient(circle, rgba(59,130,246,0.5) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-lg h-px bg-gradient-to-r from-transparent via-[#3B82F6]/20 to-transparent" />

      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tight mb-6">
            Need help with the systems behind your business?
          </h2>
          <p className="text-lg md:text-xl text-slate-400 mb-10 leading-relaxed">
            Tell us what you are running today, where the problems are and what the business needs
            next. We can help identify the infrastructure approach that fits.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              href="/services"
              className="inline-flex items-center gap-2.5 px-8 py-4 rounded-xl font-semibold text-slate-200 border border-white/15 hover:border-[#3B82F6]/50 hover:text-[#3B82F6] transition-all duration-300 whitespace-nowrap cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:ring-offset-2 focus:ring-offset-[#060F1E]"
            >
