...
export default function EmailErrorPage({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hostname === 'localhost') {
      console.error('[admin:email] error boundary caught', error.message);
    }
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center px-4">
      ...
      <h1 className="text-xl font-bold text-white mb-2">Email Studio Error</h1>
      <p className="text-sm text-slate-400 mb-8 max-w-md text-center">
        An unexpected error occurred while loading the Email Studio. Your admin session is still active.
      </p>
