    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-16 bg-[#1E293B] border border-[rgba(255,255,255,0.06)] rounded-xl animate-pulse" />)}
        </div>
      ) : sorted.length === 0 ? (
        ...
        <div className="space-y-2">
          {sorted.map(a => (
            <div key={a.id} className="bg-[#1E293B] border border-[rgba(255,255,255,0.06)] rounded-2xl p-5 hover:border-[rgba(255,255,255,0.12)] transition-all">
                    ...
                    <AnimatePresence>
                      {menuOpenId === a.id && (
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                          className="absolute right-0 top-full mt-1 w-40 bg-[#1E293B] border border-[rgba(255,255,255,0.1)] rounded-xl shadow-xl z-30 overflow-hidden">
                          <div className="p-1">
